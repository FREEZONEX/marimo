...
class S3TablesConnection:
    ...
    def __init__(
        self, conn: duckdb.DuckDBPyConnection, credential_method: str = None
    ):
        self._conn = conn
    ...
    def _refresh_credentials_boto3(self) -> bool:
        ...
        try:
            ...
            if credentials is None:
                print(
                    "[tier0_s3tables] boto3 returned no credentials",
                    file=sys.stderr,
                )
                return False
            ...
            self._last_refresh = time.time()
            print(
                "[tier0_s3tables] Credentials refreshed (boto3)",
                file=sys.stderr,
            )
            return True
            ...
            print(
                f"[tier0_s3tables] Error refreshing credentials: {e}",
                file=sys.stderr,
            )
            return False
...
def _refresh_credentials_boto3(conn: duckdb.DuckDBPyConnection) -> bool:
    ...
    try:
        ...
        print(
            "[tier0_s3tables] Credentials refreshed (boto3)", file=sys.stderr
        )
        return True
    except Exception as e:
        print(
...
def _start_background_refresh(
    ...
    # 防止重复启动刷新线程
    if getattr(builtins, "_tier0_refresh_thread_started", False):
        return
...
def _attach_s3tables(conn: duckdb.DuckDBPyConnection) -> str | None:
    ...
    if credential_method is None:
        try:
            ...
            print(
                f"[tier0_s3tables] explicit credentials failed: {e}",
                file=sys.stderr,
            )
...
def _create_connection():
    ...
    if hasattr(builtins, "_tier0_s3conn"):
        ...
        credential_method = getattr(
            builtins, "_tier0_credential_method", "unknown"
        )
        print(
