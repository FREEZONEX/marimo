...
class PackagesCallbacks:
    ...
    def register(self, router: RequestRouter) -> None:
        ...
        router.register(
            RefreshInstalledModulesCommand,
            self._handle_refresh_installed_modules,
        )
    ...
    async def _handle_refresh_installed_modules(
        self, request: RefreshInstalledModulesCommand
    ) -> None:
        await self.refresh_installed_modules(request.modules)
    ...
    async def refresh_installed_modules(self, modules: list[str]) -> None:
        """Refresh the kernel after packages were installed externally."""
        if not modules:
            return
